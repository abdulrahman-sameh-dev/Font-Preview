...
const columns: ColumnDef<Item>[] = [
    ...
    cell: ({ row }) => {
      return (
        <div className="flex items-center">
          <span>{row.getValue("Unicode")}</span>
        </div>
      );
    },
    size: 120,
  ...
  {
    header: "Copy", // العمود الجديد الذي يحتوي على زر النسخ
    accessorKey: "copy",
    cell: ({ row }) => {
      ...
      const copyToClipboard = async () => {
        try {
          ...
          console.log("Unicode copied to clipboard:", unicodeValue);
        } catch (error) {
...
export default function Component() {
  ...
  return (
    <div style={{ fontFamily: 'eaalim-font' }} className="space-y-4 p-4 max-sm:w-[305px]">
      {/* Filters */}
